...
import { FeatherIcon, FontAwesome5Icon, MaterialCommunityIcon, MaterialIcons, } from '../../../../utils/Icons'
...
const Transactions = () => {
    ...
    const { data: transactionData, isLoading: refreshing, refetch } = useFetchTransaction(bookId)
    const { data: friendData, refetch: friendRefetch, isLoading: fetching } = useFetchFriend(bookId)
    ...
    const filteredTransactions = filter === 'all'
        ...
        : transactionData?.transactions?.filter((transaction: any) => transaction.type === filter);
    ...
    const handlePress = () => {
        if (bookType === 'group') {
            return navigation.navigate('AddTransaction', { type: 'expense', bookType, bookId })
        }
    ...
    const FloatingActionButton = ({ isExpanded, index, buttonLetter, type }: any) => {
    ...
    return (
        <View style={styles.root}>
            <AuthHeader title={`${bookTitle}'s Transactions`}
                showRightIcon={true}
                rightIconName="chart-bar"
                onRightIconPress={() => navigation.navigate('InnerScreen', {
                ...
                <View style={styles.topSection}>
                    <View style={styles.innerTop}>
                        <View style={styles.card}>
                            ...
                            <Text style={[styles.price, { fontSize: bookType === 'group' ? 28 : 32 }]}>Rs. {bookType === 'group' ? getPositiveNumber(transactionData?.totals?.balance) : transactionData?.totals?.balance}</Text>
                        </View>
                    </View>

                    {bookType === 'single' && <View style={styles.row}>
                        {IncomeExpense.map((finance: { type: string, total: string }) => (
                            <View style={styles.typeContainer} key={finance.type}>
                                ...
                                <View style={styles.typeInnerContainer}>
                                    <FeatherIcon
                                        size={22}
                                        color={finance.type.toLowerCase() === 'income' ? theme.SUCCESS : theme.ERROR}
                                        name={finance.type.toLowerCase() === 'income' ? 'arrow-up' : 'arrow-down'}
                                    />
                                    <Text style={[styles.typePrice, { color: finance.type.toLowerCase() === 'income' ? theme.SUCCESS : theme.ERROR }]}>Rs. {finance.total}</Text>
                                </View>
                            </View>
                        ))}
                    </View>}
                    ...
                    <FlatList
                        ...
                        refreshControl={
                            <RefreshControl
                                ...
                                onRefresh={() => { friendRefetch(); refetch() }}
