...
import { useCreateFinancialBook, useDeleteFinancialBook, useUpdateFinancialBook } from '../../../../../ReactQueryHook/book.hook';
...
const BookFormSheet: React.FC<Props> = ({
    ...
    console.log('onClose', selectedBook)
    const [newBookTitle, setNewBookTitle] = useState('');
    const [selectedBookType, setSelectedBookType] = useState<'single' | 'group'>('single');
    const { mutate: createFinancialBook, isLoading: isCreating } = useCreateFinancialBook()
    const { mutate: updateFinancialBook, isLoading: isUpdating } = useUpdateFinancialBook()
    const { mutate: deleteFinancialBook } = useDeleteFinancialBook(selectedBook.title)
    ...
    const onSubmit = () => {
        ...
        if (isAddMode) {
            ...
            updateFinancialBook({ id: selectedBook.id, ...data }, {
                onSuccess: () => {
                    resetForm();
                    onClose();
                },
                ...
                <View style={styles.modalButtonContainer}>
                    ...
                    <TouchableOpacity style={styles.modalButton} onPress={onSubmit} >
                        {(isCreating || isUpdating) ?
                            <Text>
                                <ActivityIndicator /> <Text style={[styles.modalButtonText, { marginLeft: 10 }]}>
                                    Saving...
                                </Text>
                            </Text>
