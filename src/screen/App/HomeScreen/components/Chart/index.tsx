...
const Chart = () => {
    ...
    const { data: chartData, isLoading: refreshing, refetch } = useFetchChartTransaction();
    // ===== MOCK DATA START =====
        ...
        if (hasNoData) {
            return (
                <View style={styles.centerLabelContainer}>
                    <Text style={[styles.statusText, { color: theme.DARK_TEXT }]}>No Data</Text>
                    <Text style={[styles.noDataText, { color: theme.DARK_TEXT }]}>Add transactions</Text>
                </View>
            );
        }

        let savingsPercentage = income > 0
            ? ((income - expense) / income) * 100
            : 0;

        // Cap the value between -100 and 100
        savingsPercentage = Math.max(Math.min(Math.round(savingsPercentage), 100), -100);
    ...
    return (
        ...
        //             radius={100}
        //             innerRadius={65}
        //             innerCircleColor={theme.LIGHT_PURPLE}
